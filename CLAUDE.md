...
- **Runtime dirs are diverted out of OneDrive (od-divert, 2026-06-09).** `.venv`, `.local`, `platform/web/{.next,out}`, and `platform/web/public/artifacts` are symlinks into `~/.local-build/valley-k-small/`. Source stays OneDrive-synced; venv and build I/O hit real local disk (fixes the numpy-`.so` mmap failures and the `next build` sync storm). If a target is missing, regenerate it in place (`python3 -m venv`, `next build`, `reportctl web-data`) — content lands outside OneDrive automatically. `platform/web/node_modules` stays a REAL directory in OneDrive on purpose: npm's reify replaces a node_modules root symlink with a real dir on every install (tried 2026-06-09, twice), and OneDrive's sync client excludes `node_modules` from upload by default. Because `.next` is a symlink, Next.js worker processes resolve runtime modules from its REALPATH — `~/.local-build/valley-k-small/node_modules` must stay symlinked to `platform/web/node_modules` or static export dies with `Cannot find module 'react/jsx-runtime'`. Do NOT divert `platform/web/public/data/v1` (contains tracked files; git does not follow symlinks).
