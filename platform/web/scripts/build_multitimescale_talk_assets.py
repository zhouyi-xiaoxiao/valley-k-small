...
def build_talk_data() -> None:
    slides = [
        ...
        },
        {
            ...
            "start": "3:05",
            "end": "4:05",
            "title": "Before the curve, there is a live 2D walk",
            "title_en": "Before the curve, there is a live 2D walk",
            "title_cn": "曲线之前，是实时二维格点跳动",
            "sentence": "The walker moves one nearest-neighbour step at a time; first-passage data is built by stopping these paths at targets.",
            "sentence_en": "The walker moves one nearest-neighbour step at a time; first-passage data is built by stopping these paths at targets.",
            "sentence_cn": "walker 每次只跳到最近邻；first-passage 数据就是把这些路径在目标处停止并计数。",
            "question_en": "What exactly is being counted when the curve rises?",
            "question_cn": "曲线升起时，我们到底在数什么？",
            "animation": {"kind": "grid-hop-lab"},
        },
        {
            ...
            "sentence": "Once the local jumping process is clear, the double-peak claim must separate routes from targets.",
            "sentence_en": "Once the local jumping process is clear, the double-peak claim must separate routes from targets.",
            "sentence_cn": "局部跳动过程讲清楚后，双峰 claim 必须进一步区分路径和 target。",
    ...
    script_en = {
        ...
        "blocks": [
            ...
            {
                "slide_id": "grid2d-live-hop",
                "start": "3:05",
                "end": "4:05",
                "title": "Before the curve, there is a live 2D walk",
                "spoken_text": "Before I talk about one target, two targets, or budgets, I want the primitive on screen. This is just a walker on a two-dimensional lattice, making one nearest-neighbour jump at a time. First-passage data is produced by repeating this process and stopping each path when it hits a target.",
                "speaker_notes": "Let the animation run for a few seconds, then toggle target count or bias once.",
                "timing_prompt": "Show the real-time lattice hopping process.",
            },
            {
                ...
                "end": "5:05",
                "title": "In Grid2D, route competition becomes geometry",
                "spoken_text": "Once that local hopping process is clear, the same double-peak language becomes richer. A second feature can represent a route, a target, or a route-target pair, so the Grid2D work keeps geometry and target count visible instead of collapsing everything into one curve.",
                "speaker_notes": "Move from the live process to spatial mechanisms.",
    ...
    script_cn = {
        ...
        "blocks": [
            ...
            for item, cn_text, cn_notes, cn_prompt in [
                ...
                (
                    script_en["blocks"][3],
                    "在讲 one target、two targets 或 budget 之前，我想先把最基本的过程放到屏幕上：这就是一个 walker 在二维格点上，每次做一步最近邻跳动。first-passage 数据就是不断重复这个过程，并在路径命中目标时停止计数。",
                    "让动画先跑几秒，然后切换一次目标数量或偏置。",
                    "展示实时二维格点跳动过程。",
                ),
                (
                    script_en["blocks"][4],
                    "这个局部跳动过程讲清楚之后，同样的双峰语言才会变复杂。第二个特征可能代表路径，也可能代表 target，或者是 route-target pair。所以 Grid2D 工作不能只看一条曲线，必须保留几何和 target count。",
                    "从实时过程过渡到空间机制。",
