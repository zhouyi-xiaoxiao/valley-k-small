#!/usr/bin/env python3
"""Fig.3-style panels and environment schematics for v12 (fig3v5)."""
...
def _draw_candidate_B_env_inset(
    ...
    *,
    ...
    if strip_view is None:
        strip_view = _candidate_B_strip_view(case, corridor_rows, pad_x=1, pad_y=0, min_width=10, min_height=4)
...
def plot_candidate_B_env_v12(
    ...
    *,
    ...
    fig = plt.figure(figsize=(8.0, 5.2), constrained_layout=False)
    gs = fig.add_gridspec(2, 1, height_ratios=[1.0, 2.2], hspace=0.08)
    fig.subplots_adjust(left=0.055, right=0.985, bottom=0.08, top=0.96)

    ax_over = fig.add_subplot(gs[0, 0])
    ax_strip = fig.add_subplot(gs[1, 0])

    _draw_candidate_B_overview(
        ax_over,
        case,
        strip_view=strip_view,
        corridor_halfwidth=corridor_halfwidth,
...
def plot_symbol_legend_v12(*, outpath: Path, dpi: int = 800) -> None:
    apply_style_v12()
...
def plot_fig3_panel_v12(
    ...
    *,
    ...
    heat_mask: np.ndarray | None = None,
    heat_q: float = 1e-4,
    heat_floor_ratio: float = 0.0,
    heat_eps: float = 1e-14,
    fast_path: np.ndarray | None = None,
    slow_path: np.ndarray | None = None,
) -> None:
    apply_style_v12()
    ...
    if env_strip:
        fig = plt.figure(figsize=(11.4, 6.2), constrained_layout=False)
        fig.subplots_adjust(left=0.055, right=0.965, bottom=0.07, top=0.96, wspace=0.12, hspace=0.12)
        outer = fig.add_gridspec(3, 3, width_ratios=[1.25, 1.55, 0.05], wspace=0.14, hspace=0.14)
        left = outer[:, 0].subgridspec(2, 1, height_ratios=[1.0, 2.2], hspace=0.06)
        ax_over = fig.add_subplot(left[0, 0])
        ax_strip = fig.add_subplot(left[1, 0])
        ax_heat = [fig.add_subplot(outer[i, 1]) for i in range(3)]
        ...
        if heat_view is None:
            heat_view = strip_view_full_width(case, corridor_rows, pad_y=6)
    ...
    def _apply_mask(mat: np.ndarray) -> np.ndarray:
        if heat_mask is None:
            return mat
        masked = np.array(mat, copy=True)
        masked[~heat_mask] = np.nan
        return masked

    mats_masked = [_apply_mask(mat) for mat in mats]
    mats_use = mats_masked
    if heat_view is not None:
        mats_use = [
    ...
    vmin, vmax = heatmap_limits(mats_use, eps=heat_eps, q=heat_q)
    if heat_floor_ratio > 0.0:
        vmin = max(vmin, vmax * heat_floor_ratio)
    if vmin >= vmax:
        vmin = max(heat_eps, vmax * 0.5)
    show_overlay = not env_strip
    for ax, mat, t in zip(ax_heat, mats_masked, times):
        plot_log_heatmap_P(ax, mat, N=case.N, vmin=vmin, vmax=vmax, eps=heat_eps, view=heat_view)
        ax.set_aspect("auto")
