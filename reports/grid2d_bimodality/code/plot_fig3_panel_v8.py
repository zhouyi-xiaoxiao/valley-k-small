#!/usr/bin/env python3
"""Fig.3-style panels and environment schematics for v8."""
...
def plot_symbol_legend_v8(*, outpath: Path, dpi: int = 600) -> None:
    apply_style_v8()
...
def plot_fig3_panel_v8(
    ...
    *,
    ...
    norm = heatmap_norm(mats, eps=eps)
    ax_h = [fig.add_subplot(gs[i, 1]) for i in range(3)]
    for ax, P, t in zip(ax_h, mats, times):
        plot_log_heatmap_P(ax, P, N=case.N, norm=norm, eps=eps)
        draw_walls(ax, case.barriers_reflect)
        if case.barriers_perm:
    ...
    cb = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap="magma"), cax=cax)
    cb.set_label("P(n,t)")
