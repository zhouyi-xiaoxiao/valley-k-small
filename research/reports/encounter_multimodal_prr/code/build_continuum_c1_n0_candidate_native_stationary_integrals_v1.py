"""Build result-blind candidate-native stationary physical integrals.

The sole authority is a canonical ASCII JSON request supplied by absolute
path.  The request binds the candidate member, reference density, formula,
configuration family, method-parameter registry, every exact partition, both
source-separated implementations, runtime versions, and the previously absent
absolute output path.  It contains no expected output or result digest.
...
OUTPUT_SCHEMA: Final = "encounter_continuum_c1_n0_candidate_native_stationary_integrals_v1"
OUTPUT_STATUS: Final = (
...
def _validate_method_registry(
    ...
    if (
        ...
        or count != len(contract.parameter_order)
...
def _load_request(request_path: Path, output_path: Path) -> tuple[dict[str, Any], Snapshot]:
    ...
    if output["schema"] != OUTPUT_SCHEMA or requested_output != output_path:
        raise CandidateStationaryFailure(HOLD_REQUEST, "request/CLI output mismatch")
    forbidden_fragments = (
        "artifact_sha",
        "expected_output",
        "expected_result",
    ...
    if any(
        fragment in key.lower() for key in _walk_keys(request) for fragment in forbidden_fragments
    ):
        raise CandidateStationaryFailure(HOLD_REQUEST, "request contains a result/observed pin")
...
def _validate_member_and_partitions(
    ...
    if not _is_sha256(member_identity_sha256) or member_identity_sha256 != _domain_digest(
...
def _publish(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        if (
            ...
            or stat.S_IMODE(finished_stage.st_mode) != 0o400
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if check:
            observed = immutable_snapshot(output_path, cap=max(MAX_JSON_BYTES, len(payload)))
            if observed.raw != payload:
                raise CandidateStationaryFailure(HOLD_OUTPUT, "read-only check mismatch")
        else:
