"""Build a static C1/n=0 runtime byte-pin inventory from trusted bytes.

This module is intentionally a pure byte-to-byte component.  It does not open,
stat, resolve, import, execute, or publish any path.  It accepts only the
literal frozen layout and byte hashes below.  A future authenticated runtime
probe is required before any runtime-closure or observed-metadata claim.

Mach-O dependency edges are reconstructed from supplied immutable bytes; they
are never copied from caller metadata.
"""
...
class RuntimeAuthorityBuildFailure(RuntimeError):
    """Fail-closed rejection while constructing a runtime authority."""


def _fail(detail: str) -> NoReturn:
    raise RuntimeAuthorityBuildFailure(detail)


def canonical_json_bytes(value: object) -> bytes:
    """Return the repository's canonical ASCII JSON encoding."""
    ...
    if len(encoded) > MAX_AUTHORITY_BYTES:
        _fail("runtime authority exceeds byte cap")
    return encoded
...
def _strict_json(
    ...
    *,
    ...
    except RuntimeAuthorityBuildFailure:
        raise
...
def _operation_model_binding(raw: bytes) -> dict[str, str]:
    model = _strict_json(
        raw,
    ...
    process_contract = model.get("process_contract")
    if type(process_contract) is not dict:
        _fail("operation model: process_contract object required")
    process_raw = canonical_json_bytes(process_contract)
    if _sha256(process_raw) != PROCESS_CONTRACT_SECTION_SHA256:
...
def _lc_string(
    ...
    *,
    ...
    if end < 0:
        _fail(f"{label}: unterminated string")
    if any(command[end + 1 :]):
        _fail(f"{label}: nonzero load-command padding")
    try:
...
def _arm64_slice(raw: bytes, label: str) -> tuple[bytes, dict[str, Any]]:
    ...
    arm64 = [entry for entry in entries if entry[0] == CPU_TYPE_ARM64]
    if len(arm64) != 1:
        _fail(f"{label}: exactly one arm64 fat slice required")
    cputype, cpusubtype, offset, size = arm64[0]
...
def _parse_macho(raw: bytes, image_id: str) -> dict[str, Any]:
    ...
    for command_index in range(ncmds):
        ...
        if command_size < 8 or command_size % 8 or cursor + command_size > command_end:
        ...
        if command_id == LC_ID_DYLIB:
            ...
            name = _lc_string(
                ...
                offset_field=8,
                minimum_offset=24,
                label=f"{image_id} LC_ID_DYLIB",
            )
            _, _, _, _, current_version, compatibility_version = struct.unpack_from(
                "<IIIIII", command, 0
            )
            dylib_id = {
            ...
            edges.append(
                {
                    ...
                    "command_index": command_index,
                    "load_path": _lc_string(
                        command,
                        "<",
                        offset_field=8,
                        minimum_offset=minimum,
                        label=f"{image_id} dependency",
                    ),
                }
            )
...
def _loader_resolve(source_path: str, load_path: str, root: str) -> str:
    suffix = load_path.removeprefix("@loader_path")
...
def _classify_edges(
    ...
    host_boundaries = {item["path"]: item["boundary_kind"] for item in HOST_BOUNDARY_EDGES}
    ...
    for edge in parsed_edges:
        ...
        if load_path.startswith("@loader_path"):
            resolved = _loader_resolve(image_path, load_path, AUTHORITY_ROOT)
        ...
        boundary_kind = host_boundaries.get(load_path)
...
def build_runtime_authority(authenticated_files: object) -> bytes:
    """Construct the canonical static byte-pin inventory."""

    snapshot = _snapshot_files(authenticated_files)
    operation_binding = _operation_model_binding(snapshot[OPERATION_MODEL_PATH])
    ...
    used_host_boundaries: set[str] = set()
    for image_id in IMAGE_IDS:
        ...
        edges, used = _classify_edges(image_id, parsed["edges"])
        used_host_boundaries.update(used)
        images.append(
            {
    ...
    expected_host_paths = {item["path"] for item in HOST_BOUNDARY_EDGES}
    if used_host_boundaries != expected_host_paths:
