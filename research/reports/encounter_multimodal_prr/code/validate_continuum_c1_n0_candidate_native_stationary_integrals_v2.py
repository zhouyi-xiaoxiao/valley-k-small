"""Independently validate a committed role-9 stationary-integral artifact.

This verifier does not import the producer or any legacy scientific module.
It independently reopens the plan/bundle/commitment chain, reconstructs all member
...
artifact bytes, then exclusively publishes an immutable validation receipt.
"""
...
ARTIFACT_SCHEMA: Final = "encounter_c1_n0_stationary_physical_integral_source_v2"
...
ROLE8_ARTIFACT_SCHEMA: Final = "encounter_c1_n0_raw_axis_formula_primitive_source_v2"
ROLE10_ARTIFACT_SCHEMA: Final = "encounter_c1_n0_killing_factor_geometry_source_v2"
...
RECEIPT_STATUS: Final = "PASS_INDEPENDENT_COMPLETE_RECOMPUTATION_AND_EXCLUSIVE_RECEIPT"
...
HOLD_RECEIPT = "HOLD_CANDIDATE_STATIONARY_VERIFY_RECEIPT"
...
_ROLE_ARTIFACT_SCHEMAS: Final = {
    8: ROLE8_ARTIFACT_SCHEMA,
    9: ARTIFACT_SCHEMA,
    10: ROLE10_ARTIFACT_SCHEMA,
}
...
def _immutable_image(
    ...
    *,
    ...
    stable_identity: tuple[int, ...] | None = None
    try:
        ...
        stable_identity = _stable_file_identity(after)
        _revalidate_anchored_image(path, directory_chain, stable_identity)
    finally:
        _close_descriptor(descriptor)
    if stable_identity is None:
        raise CandidateStationaryVerificationFailure(
            HOLD_IMMUTABLE, f"immutable input identity unavailable: {path}"
        )
    raw = b"".join(chunks)
    return FileImage(path, raw, _sha256(raw), directory_chain, stable_identity)


def _revalidate_file_image(image: FileImage) -> None:
    _revalidate_anchored_image(
        image.path,
        image.directory_chain,
        image.stable_identity,
    )
...
def _read_runtime_pin(
    pin: Any,
    label: str,
    *,
    cap: int = MAX_RUNTIME_BYTES,
) -> FileImage:
    current = _keys(pin, _PIN_KEYS, HOLD_RUNTIME, label)
    path = _absolute(current["path"], HOLD_RUNTIME, label)
    if not _valid_sha(current["sha256"]):
        raise CandidateStationaryVerificationFailure(HOLD_RUNTIME, f"{label}: invalid SHA-256")
    image = _immutable_image(path, cap=cap, require_read_only=False)
    if image.sha256 != current["sha256"]:
        raise CandidateStationaryVerificationFailure(HOLD_RUNTIME, f"{label}: SHA-256 mismatch")
    return image
...
def _select_methods(registry: dict[str, Any], selection: dict[str, Any]) -> SelectedMethods:
    ...
    for identifier in (primary_id, sentinel_id, exact_id):
        if canonical_bytes(by_id[identifier]["parameters"]) != canonical_bytes(
            ...
            raise CandidateStationaryVerificationFailure(
                HOLD_METHOD, "selected parameter record mismatch"
            )
...
def _all_string_values(
    ...
    if type(value) is str:
        ...
        for key, item in value.items():
            result.extend(_all_string_values(item, key))
    elif type(value) is list:
        for item in value:
            result.extend(_all_string_values(item, field))
    return result
...
def _require_false_boundary(value: Any, label: str) -> dict[str, bool]:
    if (
        ...
        raise CandidateStationaryVerificationFailure(
            HOLD_REQUEST, f"{label}: false-only boundary required"
        )
    return value


def _context_pin(value: Any, label: str) -> dict[str, str]:
    pin = _keys(value, _CONTEXT_PIN_KEYS, HOLD_REQUEST, label)
    if not _is_nonempty_string(pin["schema"]) or not _valid_sha(pin["sha256"]):
        raise CandidateStationaryVerificationFailure(
            HOLD_REQUEST, f"{label}: invalid contextual pin"
        )
    path = pin["path"]
...
def _same_pin(left: Any, right: Any, label: str) -> None:
    if _keys(left, _PIN_KEYS, HOLD_REQUEST, f"{label} left") != _keys(
        right, _PIN_KEYS, HOLD_REQUEST, f"{label} right"
    ):
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, f"{label}: pin mismatch")


def _same_logical_authority(absolute_pin: Any, logical_pin: Any, label: str) -> None:
    execution = _keys(absolute_pin, _PIN_KEYS, HOLD_REQUEST, f"{label} execution")
    logical = _keys(logical_pin, _PIN_KEYS, HOLD_REQUEST, f"{label} logical")
    absolute = _absolute(execution["path"], HOLD_REQUEST, f"{label} execution path")
...
def _plan_pin_shape(value: Any, label: str) -> tuple[dict[str, str], FileImage]:
    pin = _keys(value, _PIN_KEYS, HOLD_REQUEST, label)
    if not _valid_sha(pin["sha256"]):
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, f"{label}: invalid SHA-256")
    path = _absolute(pin["path"], HOLD_REQUEST, f"{label} path")
    image = _immutable_image(path)
...
def _validate_role8_runtime_closure(
    closure_image: FileImage,
    label: str,
) -> tuple[dict[str, FileImage], tuple[FileImage, ...]]:
    closure = _decode_canonical(closure_image.raw, f"{label} closure content")
    _keys(
        closure,
        _ROLE8_RUNTIME_CLOSURE_KEYS,
        HOLD_RUNTIME,
        f"{label} closure content",
    )
    ...
    if (
        ...
        raise CandidateStationaryVerificationFailure(
            HOLD_RUNTIME, f"{label}: schema/status mismatch"
        )
    claims = _keys(
    ...
    if not _json_exactly_equal(claims, _ROLE8_RUNTIME_CLAIMS):
        raise CandidateStationaryVerificationFailure(HOLD_RUNTIME, f"{label}: claim mismatch")
    if (
        type(closure["report_local_dependencies"]) is not list
        or closure["report_local_dependencies"]
    ):
        raise CandidateStationaryVerificationFailure(
            HOLD_RUNTIME, f"{label}: report-local dependency mismatch"
        )
    ...
    source_images = {
        role: _read_pin(pin, f"{label} {role} source") for role, pin in code_inputs.items()
    }
    if source_images["producer"].path == source_images["verifier"].path:
        raise CandidateStationaryVerificationFailure(
            HOLD_RUNTIME, f"{label}: source separation mismatch"
        )

    native_runtime = _keys(
        closure["native_runtime"],
        _RUNTIME_KEYS,
        HOLD_RUNTIME,
        f"{label} native runtime",
    )
    if any(
        type(value) is not str or not value for value in native_runtime.values()
    ) or not _json_exactly_equal(native_runtime, _runtime()):
        raise CandidateStationaryVerificationFailure(
            HOLD_RUNTIME, f"{label}: native runtime mismatch"
        )
    ...
    if executable.path != Path(sys.executable).resolve():
        raise CandidateStationaryVerificationFailure(
            HOLD_RUNTIME, f"{label}: Python executable mismatch"
        )
    if not _json_exactly_equal(closure["python_imports"], _ROLE8_PYTHON_IMPORTS):
        raise CandidateStationaryVerificationFailure(
            HOLD_RUNTIME, f"{label}: Python imports mismatch"
        )
    ...
    if type(native_libraries) is not list or len(native_libraries) != len(
        ...
        raise CandidateStationaryVerificationFailure(HOLD_RUNTIME, f"{label}: native library count")
    ...
    for ordinal, raw_pin in enumerate(native_libraries):
        ...
        native_image = _read_runtime_pin(
            {"path": pin["path"], "sha256": pin["sha256"]},
            f"{label} native library {expected_role}",
        )
        if native_image.path != expected_candidates[expected_role][0].resolve():
            raise CandidateStationaryVerificationFailure(
                HOLD_RUNTIME, f"{label}: native library path mismatch"
            )
    ...
    if len({image.path for image in all_images}) != len(all_images):
        raise CandidateStationaryVerificationFailure(HOLD_RUNTIME, f"{label}: runtime path alias")
    return source_images, all_images


def _validate_plan_partition_bindings(
    value: Any,
    label: str,
) -> tuple[list[dict[str, Any]], tuple[FileImage, ...]]:
    if type(value) is not list or len(value) != EXPECTED_AXIS_COUNT:
        raise CandidateStationaryVerificationFailure(
            HOLD_REQUEST, f"{label}: partition cardinality"
        )
    ...
    for ordinal, raw_pin in enumerate(value):
        ...
        expected_index, coordinate_ordinal = divmod(ordinal, len(AXIS_ORDER))
        coordinate = AXIS_ORDER[coordinate_ordinal]
        relative = pin["member_report_relative_path"]
        pure = PurePosixPath(relative) if _is_nonempty_string(relative) else None
        absolute = _absolute(pin["path"], HOLD_REQUEST, f"{label} partition path {ordinal}")
        if (
...
def _validate_plan_entry_semantics(
    ...
    *,
    ...
    if (
        ...
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, f"{label}: identity mismatch")

    planned_request = _keys(entry["request"], _ENTRY_REQUEST_KEYS, HOLD_REQUEST, f"{label} request")
    request_path = _absolute(planned_request["path"], HOLD_REQUEST, f"{label} request path")
    if planned_request != {
        "path": str(request_path),
        "schema": _ROLE_REQUEST_SCHEMAS[role],
        "status": REQUEST_STATUS,
    }:
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, f"{label}: request contract")

    outputs = _keys(entry["outputs"], _ENTRY_OUTPUT_KEYS, HOLD_REQUEST, f"{label} outputs")
    artifact = _keys(outputs["artifact"], _OUTPUT_KEYS, HOLD_REQUEST, f"{label} artifact")
    receipt = _keys(
    ...
    if (
        artifact["schema"] != _ROLE_ARTIFACT_SCHEMAS[role]
        or receipt["schema"] != _ROLE_RECEIPT_SCHEMAS[role]
        or len({request_path, artifact_path, receipt_path}) != 3
    ):
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, f"{label}: output contract")
    ...
    if role == 8:
        ...
        if closure_pin["schema"] != ROLE8_RUNTIME_CLOSURE_SCHEMA or not _valid_sha(
            closure_pin["sha256"]
        ):
            raise CandidateStationaryVerificationFailure(
                HOLD_REQUEST, f"{label}: runtime closure pin"
            )
        closure_path = _absolute(closure_pin["path"], HOLD_REQUEST, f"{label} runtime closure path")
        closure_image = _immutable_image(closure_path)
        if closure_image.sha256 != closure_pin["sha256"]:
            raise CandidateStationaryVerificationFailure(
                HOLD_INPUT, f"{label}: runtime closure SHA-256 mismatch"
            )
        source_images, runtime_images = _validate_role8_runtime_closure(
            closure_image,
            f"{label} runtime closure",
        )
        producer_path = source_images["producer"].path
        verifier_path = source_images["verifier"].path
        ...
        _, producer_image = _plan_pin_shape(closure["producer"], f"{label} producer source")
        _, verifier_image = _plan_pin_shape(closure["verifier"], f"{label} verifier source")
        producer_path = producer_image.path
        verifier_path = verifier_image.path
        dependency_images.extend((producer_image, verifier_image))
        required_runtime = _keys(
            closure["runtime_requirements"],
            _RUNTIME_KEYS,
            HOLD_REQUEST,
            f"{label} runtime requirements",
        )
        if required_runtime != _runtime():
            raise CandidateStationaryVerificationFailure(HOLD_RUNTIME, f"{label}: runtime mismatch")
        expected_prefix = [sys.executable]

    invocations = _keys(
        entry["invocations"], _INVOCATIONS_KEYS, HOLD_REQUEST, f"{label} invocations"
    )
    ...
    for invocation_role in ("producer", "verifier"):
        ...
        if (
            ...
            raise CandidateStationaryVerificationFailure(
                HOLD_REQUEST, f"{label}: invocation type mismatch"
            )
        _absolute(
            invocation["cwd"],
            HOLD_REQUEST,
    ...
    if (
        ...
        raise CandidateStationaryVerificationFailure(
            HOLD_REQUEST, f"{label}: exact invocation mismatch"
        )
    ...
    for authority_role, authority_pin in authorities.items():
        _, authority_image = _plan_pin_shape(authority_pin, f"{label} authority {authority_role}")
        dependency_images.append(authority_image)
    context_role_map = {
    ...
    for authority_role, context_role in context_role_map.items():
        _same_logical_authority(
            ...
            _context_pin(shared_context[context_role], f"shared {context_role}"),
    ...
    if role == 8:
        if type(method_selection) is not list or method_selection != _ROLE8_METHOD_SELECTION:
            raise CandidateStationaryVerificationFailure(HOLD_METHOD, f"{label}: method selection")
    ...
    _, partition_images = _validate_plan_partition_bindings(entry["partition_path_bindings"], label)
    dependency_images.extend(partition_images)
    return (request_path, artifact_path, receipt_path), tuple(dependency_images)
...
) -> tuple[tuple[Path, ...], tuple[Path, ...], tuple[FileImage, ...]]:
    request_paths: list[Path] = []
    output_paths: list[Path] = []
    dependency_images: list[FileImage] = []
    reference_partitions: bytes | None = None
    for role, entry in zip((8, 9, 10), entries, strict=True):
        slots, entry_images = _validate_plan_entry_semantics(
            entry,
            role=role,
        ...
        dependency_images.extend(entry_images)
        ...
        if reference_partitions is None:
            ...
            raise CandidateStationaryVerificationFailure(
                HOLD_REQUEST, "roles 8--10 partition bindings differ"
            )
    all_slots = [*request_paths, *output_paths]
    if len(all_slots) != 9 or len(set(all_slots)) != 9:
        raise CandidateStationaryVerificationFailure(
            HOLD_REQUEST, "roles 8--10 replay slots collide"
        )
    dependency_by_path: dict[Path, FileImage] = {}
    for image in dependency_images:
        previous = dependency_by_path.get(image.path)
        if previous is not None and (
            previous.sha256 != image.sha256
            or previous.stable_identity != image.stable_identity
            or previous.directory_chain != image.directory_chain
        ):
            raise CandidateStationaryVerificationFailure(
                HOLD_IMMUTABLE, "plan dependency changed during nested validation"
            )
        dependency_by_path[image.path] = image
    if set(output_paths) & set(dependency_by_path):
        raise CandidateStationaryVerificationFailure(
            HOLD_REQUEST, "planned output aliases an input"
        )
    for path in output_paths:
        if path == selected_artifact_path:
            continue
        if os.path.lexists(path):
            raise CandidateStationaryVerificationFailure(
                HOLD_REQUEST, "planned output slot is not fresh"
            )
    return (
        tuple(request_paths),
        tuple(output_paths),
        tuple(dependency_by_path[path] for path in sorted(dependency_by_path, key=str)),
    )
...
def _validate_result_blind_keys(value: Any, label: str) -> None:
    ...
    _validate_result_blind_string_values(value, HOLD_REQUEST, label)


def _validate_result_blind_string_values(value: Any, code: str, label: str) -> None:
    for field, string in _all_string_values(value):
...
def _load_request(
    request_path: Path, artifact_path: Path, receipt_path: Path
) -> tuple[dict[str, Any], ReplayProtocol, dict[str, FileImage]]:
    ...
    if _keys(request["role"], _ROLE_KEYS, HOLD_REQUEST, "request role") != {
        "role_id": ROLE_ID,
        "role_name": ROLE_NAME,
    }:
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "request role")
    plan_image = _read_pin(request["plan"], "replay plan")
    plan = _decode_canonical(plan_image.raw, "replay plan")
    _keys(plan, _PLAN_KEYS, HOLD_REQUEST, "replay plan")
    _validate_result_blind_keys(plan, "replay plan")
    if plan["schema"] != PLAN_SCHEMA or plan["status"] != PLAN_STATUS:
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "replay plan boundary")
    plan_claims = _require_false_boundary(plan["claim_boundary"], "replay plan")
    if set(plan_claims) != _PLAN_BUNDLE_CLAIM_KEYS:
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "plan claim keys")
    shared_context = _keys(
    ...
    for key in (
        ...
        _context_pin(shared_context[key], f"shared {key}")
    if (
        ...
        or not _valid_sha(shared_context["configuration_row_inventory_sha256"])
        or not _valid_sha(shared_context["partition_inventory_sha256"])
        ...
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "shared authority mismatch")
    precommit_digest = _digest(PRECOMMIT_CONTEXT_DOMAIN, shared_context)
    if (
        plan["shared_precommit_context_sha256"] != precommit_digest
        or request["shared_precommit_context_sha256"] != precommit_digest
    ):
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "precommit context digest")
    ...
    if (
        ...
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "plan role ordering")
    for ordered_entry in entries:
        _keys(
            ordered_entry,
            _PLAN_ENTRY_KEYS,
            HOLD_REQUEST,
            f"role-{ordered_entry['role']} plan entry",
        )
        ordered_projection = {
            key: value
        ...
        if ordered_entry["precommit_projection_sha256"] != _digest(
            PRECOMMIT_PROJECTION_DOMAIN, ordered_projection
        ):
            raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "ordered role projection")
    planned_request_paths, planned_output_paths, plan_dependency_images = (
        _validate_all_plan_entries(
            entries,
            shared_context,
    ...
    if set(planned_output_paths) & {
        ...
        *planned_request_paths,
        ...
        raise CandidateStationaryVerificationFailure(
            HOLD_REQUEST, "planned output aliases protocol input"
        )
    entry = entries[1]
    projection = {
        key: value for key, value in entry.items() if key != "precommit_projection_sha256"
    }
    if entry["precommit_projection_sha256"] != _digest(PRECOMMIT_PROJECTION_DOMAIN, projection):
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "entry projection")
    if _keys(entry["request"], _ENTRY_REQUEST_KEYS, HOLD_REQUEST, "planned request") != {
        "path": str(request_path),
        "schema": REQUEST_SCHEMA,
        "status": REQUEST_STATUS,
    }:
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "planned request")
    outputs = _keys(entry["outputs"], _ENTRY_OUTPUT_KEYS, HOLD_REQUEST, "outputs")
    artifact_output = _keys(outputs["artifact"], _OUTPUT_KEYS, HOLD_REQUEST, "artifact output")
    receipt_output = _keys(
        outputs["validation_receipt"], _OUTPUT_KEYS, HOLD_REQUEST, "receipt output"
    )
    if (
        artifact_output["schema"] != ARTIFACT_SCHEMA
        or receipt_output["schema"] != RECEIPT_SCHEMA
        or _absolute(artifact_output["path"], HOLD_REQUEST, "artifact output") != artifact_path
        or _absolute(receipt_output["path"], HOLD_REQUEST, "receipt output") != receipt_path
        or artifact_path == receipt_path
        or not os.path.lexists(artifact_path)
        or os.path.lexists(receipt_path)
    ):
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "planned output slots")
    runtime_closure = _keys(
    ...
    runtime_required = _keys(
        runtime_closure["runtime_requirements"], _RUNTIME_KEYS, HOLD_REQUEST, "runtime"
    )
    invocations = _keys(entry["invocations"], _INVOCATIONS_KEYS, HOLD_REQUEST, "invocations")
    for label, invocation in invocations.items():
        _keys(invocation, _INVOCATION_KEYS, HOLD_REQUEST, f"{label} invocation")
        if (
            ...
            raise CandidateStationaryVerificationFailure(HOLD_REQUEST, f"{label} invocation type")
    producer_path = _absolute(runtime_closure["producer"]["path"], HOLD_REQUEST, "producer path")
    verifier_path = _absolute(runtime_closure["verifier"]["path"], HOLD_REQUEST, "verifier path")
    if (
        invocations["producer"]["argv"]
        != [
            sys.executable,
            str(producer_path),
            "--request",
        ...
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "planned invocations")
    authorities = _keys(
        entry["input_authorities"], _INPUT_AUTHORITY_ROLES, HOLD_REQUEST, "authorities"
    )
    context_roles = {
        "anti_vacuity_policy": "anti_vacuity_policy",
        "configuration": "configuration",
    ...
    for authority_role, context_role in context_roles.items():
        _same_logical_authority(
            authorities[authority_role],
            _context_pin(shared_context[context_role], f"shared {context_role}"),
            f"entry/shared {authority_role}",
        )
    commitment_image = _read_pin(request["external_predecessor_commitment"], "external commitment")
    commitment = _decode_canonical(commitment_image.raw, "external commitment")
    _keys(commitment, _COMMITMENT_KEYS, HOLD_REQUEST, "external commitment")
    _validate_result_blind_string_values(commitment, HOLD_REQUEST, "external commitment")
    bundle_image = _read_pin(commitment["candidate_bundle"], "candidate bundle")
    bundle = _decode_canonical(bundle_image.raw, "candidate bundle")
    _keys(bundle, _BUNDLE_KEYS, HOLD_REQUEST, "candidate bundle")
    _validate_result_blind_keys(bundle, "candidate bundle")
    if bundle["schema"] != BUNDLE_SCHEMA or bundle["status"] != BUNDLE_STATUS:
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "bundle boundary")
    bundle_claims = _require_false_boundary(bundle["claim_boundary"], "candidate bundle")
    if set(bundle_claims) != _PLAN_BUNDLE_CLAIM_KEYS:
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "bundle claim keys")
    _same_pin(bundle["replay_plan"], request["plan"], "bundle plan")
    ...
    if bundle["shared_precommit_context_sha256"] != precommit_digest:
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "bundle context")
    if commitment["schema"] != COMMITMENT_SCHEMA or commitment["status"] != COMMITMENT_STATUS:
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "commitment boundary")
    ...
    authority = _keys(commitment["authority"], _AUTHORITY_KEYS, HOLD_REQUEST, "authority")
    ordering = _keys(commitment["ordering"], _ORDERING_KEYS, HOLD_REQUEST, "ordering")
    claims = _keys(
    ...
    if (
        ...
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "commitment structure")
    _same_pin(
        commitment["candidate_bundle"],
        {"path": str(bundle_image.path), "sha256": bundle_image.sha256},
        "commitment bundle",
    )
    if commitment["commitment_message_sha256"] != _digest(
        COMMITMENT_MESSAGE_DOMAIN,
        {
            "authority": authority,
        ...
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "commitment message")
    ...
    if request["shared_replay_context_sha256"] != replay_digest:
        raise CandidateStationaryVerificationFailure(HOLD_REQUEST, "replay context")
    if set(planned_output_paths) & {
        request_image.path,
        plan_image.path,
        bundle_image.path,
        ...
        *planned_request_paths,
        *(image.path for image in plan_dependency_images),
    }:
        raise CandidateStationaryVerificationFailure(
            HOLD_REQUEST, "planned output aliases replay input"
        )
    expanded = {
        "code_inputs": {
        ...
        "runtime_requirements": runtime_required,
        "schema": REQUEST_SCHEMA,
        "status": REQUEST_STATUS,
    }
    images = {
        role: _read_pin(pin, role)
        for role, pin in {
            **authorities,
            "producer": runtime_closure["producer"],
            "verifier": runtime_closure["verifier"],
    ...
    if (
        images["member_spec"].sha256 != MEMBER_SHA256
        or images["method_parameters"].sha256 != PARAMETER_SHA256
    ):
        raise CandidateStationaryVerificationFailure(
            HOLD_INPUT, "accepted member/registry pin mismatch"
        )
    ...
    return expanded, protocol, images
...
def _member_partitions(
    ...
    partition_images: dict[str, FileImage] = {}
    ...
    for index, (row, binding, label, semantic) in enumerate(
        ...
        for coordinate, axis_binding in zip(AXIS_ORDER, axes, strict=True):
            ...
            partition_images[f"partition:{index}:{coordinate}"] = image
    ...
    if member_identity_sha256 != MEMBER_IDENTITY_SHA256 or member_identity_sha256 != _digest(
    ...
    return loaded_rows, output_pins, partition_images
...
def _partition_inventory(member: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, binding in enumerate(member["n0_sequence_bindings"]):
        for coordinate, axis in zip(AXIS_ORDER, binding["n0_axes"], strict=True):
            records.append(
                {
...
def _partition_closure(
    ...
    nodes: list[dict[str, str]] = [
        ...
        },
        {
            ...
            "payload_sha256": _sha256(canonical_bytes(configuration)),
        },
    ]
    ...
    for index, row in enumerate(configuration["configurations"]):
        ...
        nodes.append(
            {
                ...
                "payload_sha256": _sha256(canonical_bytes(row)),
            }
        )
        edges.append(
            {
        ...
        for axis_index, coordinate in enumerate(AXIS_ORDER):
            record = inventory[3 * index + axis_index]
            partition_id = f"03:partition:{index:02d}:{axis_index}:{coordinate}"
            nodes.append(
                {
                    "id": partition_id,
                    "kind": "exact_axis_partition",
                    "payload_sha256": _digest(PARTITION_INVENTORY_DOMAIN, record),
                }
            )
            edges.append(
                {
    ...
    return {
        ...
        "nodes": nodes,
        "sha256": _digest(DAG_DOMAIN, payload),
    }
    ...
    for row in rows:
        ...
        for axis_index, axis in enumerate(row["axes"]):
            ...
            for cell in axis["M_x_pi_cell_intervals"]:
                records.append({**identity, **cell, "record_type": "M_x_pi_cell"})
...
def _stream_digest(records: Sequence[dict[str, Any]]) -> str:
    state = hashlib.sha256()
    state.update(STREAM_DOMAIN.encode("ascii"))
    state.update(b"\0")
    state.update(len(records).to_bytes(8, "big"))
...
def _expected_artifact(
    request_path: Path, artifact_path: Path, receipt_path: Path
) -> tuple[bytes, dict[str, Any], ReplayProtocol, dict[str, FileImage]]:
    request, protocol, images = _load_request(request_path, artifact_path, receipt_path)
    shared_context = protocol.plan["shared_context"]
    anti_vacuity_policy = _decode_canonical(
        images["anti_vacuity_policy"].raw, "anti-vacuity policy"
    )
    if (
        anti_vacuity_policy.get("schema") != ANTI_VACUITY_POLICY_SCHEMA
        or images["anti_vacuity_policy"].sha256 != ANTI_VACUITY_POLICY_SHA256
    ):
        raise CandidateStationaryVerificationFailure(HOLD_INPUT, "anti-vacuity binding")
    ...
    rows_and_partitions, partition_pins, partition_images = _member_partitions(
        request, member, reference, configuration
    )
    images.update(partition_images)
    ...
    if (
        ...
        or _digest(CONFIGURATION_INVENTORY_DOMAIN, configuration_inventory)
        != shared_context["configuration_row_inventory_sha256"]
        or _digest(PARTITION_INVENTORY_DOMAIN, partition_inventory)
        != shared_context["partition_inventory_sha256"]
    ):
        raise CandidateStationaryVerificationFailure(HOLD_MEMBER, "inventory digests")
    ...
    axis_stream = {
        ...
        "sha256": _stream_digest(stream_records),
    }
    artifact = {
        ...
        "member_binding": {
            ...
            "path": str(images["member_spec"].path),
            "schema": MEMBER_SCHEMA,
            "sha256": images["member_spec"].sha256,
        },
        "method_binding": {
            "aggregation": "exact_Fraction_endpoint_sums_and_nonnegative_products",
            "dense_tensor_materialized": False,
            ...
            "registry": {
                "path": str(images["method_parameters"].path),
                "schema": PARAMETER_SCHEMA,
                "sha256": images["method_parameters"].sha256,
            },
        },
        "partition_closure": partition_closure,
        "replay_binding": {
            "candidate_bundle": {
                "path": str(protocol.bundle_image.path),
                "sha256": protocol.bundle_image.sha256,
            },
            "external_predecessor_commitment": {
                "path": str(protocol.commitment_image.path),
                "sha256": protocol.commitment_image.sha256,
            },
            "replay_plan": {
                "path": str(protocol.plan_image.path),
                "sha256": protocol.plan_image.sha256,
            },
            "request": {
                "path": str(protocol.request_image.path),
                "sha256": protocol.request_image.sha256,
            },
    ...
    return canonical_bytes(artifact), artifact, protocol, images
...
def _exclusive_publish_receipt(
    ...
    *,
    ...
    if (
        ...
        raise CandidateStationaryVerificationFailure(HOLD_RECEIPT, "receipt target not fresh")
    flags_directory = os.O_RDONLY | os.O_CLOEXEC | os.O_DIRECTORY | os.O_NOFOLLOW
    parent_descriptor: int | None = None
    stage_descriptor: int | None = None
    final_descriptor: int | None = None
    stage_name = f".{path.name}.{os.getpid()}.{os.urandom(12).hex()}.stage"
    stage_identity: tuple[int, int] | None = None
    final_identity: tuple[int, int] | None = None
    installed = False
    try:
        parent_descriptor = os.open(path.parent, flags_directory)
        ...
        if (
            ...
            raise CandidateStationaryVerificationFailure(HOLD_RECEIPT, "receipt parent contract")
        ...
        if (
            ...
            raise CandidateStationaryVerificationFailure(HOLD_RECEIPT, "receipt stage contract")
        offset = 0
        while offset < len(payload):
            written = os.write(stage_descriptor, payload[offset:])
            if written <= 0:
                raise CandidateStationaryVerificationFailure(HOLD_RECEIPT, "receipt short write")
            offset += written
        os.fsync(stage_descriptor)
        os.fchmod(stage_descriptor, 0o444)
        finished = os.fstat(stage_descriptor)
        if (
            (finished.st_dev, finished.st_ino) != stage_identity
            or finished.st_size != len(payload)
            or finished.st_nlink != 1
            or finished.st_uid != os.getuid()
            or stat.S_IMODE(finished.st_mode) != 0o444
        ):
            raise CandidateStationaryVerificationFailure(HOLD_RECEIPT, "receipt stage drift")
        ...
        if final_identity != stage_identity or final_stat.st_nlink != 2:
            raise CandidateStationaryVerificationFailure(HOLD_RECEIPT, "receipt link identity")
        raw = b""
        while len(raw) < len(payload):
            chunk = os.read(final_descriptor, len(payload) - len(raw))
            if not chunk:
                break
            raw += chunk
        if raw != payload or os.read(final_descriptor, 1):
            raise CandidateStationaryVerificationFailure(HOLD_RECEIPT, "receipt readback")
        ...
        if (
            ...
            raise CandidateStationaryVerificationFailure(HOLD_RECEIPT, "receipt final contract")
        os.fsync(parent_descriptor)
        live_parent = path.parent.stat()
        if (live_parent.st_dev, live_parent.st_ino) != (
            parent_before.st_dev,
            parent_before.st_ino,
        ):
            raise CandidateStationaryVerificationFailure(HOLD_RECEIPT, "receipt parent changed")
        if revalidate is not None:
            revalidate()
    except BaseException as error:
        if parent_descriptor is not None:
            if stage_identity is not None:
                try:
                    ...
                    if (stage_live.st_dev, stage_live.st_ino) == stage_identity:
                        os.unlink(stage_name, dir_fd=parent_descriptor)
                except OSError:
                    pass
            if installed and final_identity is not None:
                try:
                    final_live = os.stat(path.name, dir_fd=parent_descriptor, follow_symlinks=False)
                    if (final_live.st_dev, final_live.st_ino) == final_identity:
        ...
        if isinstance(error, CandidateStationaryVerificationFailure):
            raise
        raise CandidateStationaryVerificationFailure(
            HOLD_RECEIPT, "exclusive receipt publication failed"
        ) from error
    finally:
        for descriptor in (final_descriptor, stage_descriptor, parent_descriptor):
...
def validate(request_path: Path, artifact_path: Path, receipt_path: Path) -> dict[str, Any]:
    expected_raw, expected, protocol, images = _expected_artifact(
        request_path, artifact_path, receipt_path
    )
    ...
    receipt = {
        ...
        "counts": {
            "axis_count": expected["summary"]["axis_count"],
            "axis_stream_record_count": expected["summary"]["axis_stream_record_count"],
            "configuration_count": expected["summary"]["configuration_count"],
            "factorized_axis_cell_count": expected["summary"]["factorized_axis_cell_count"],
            "partition_closure_edge_count": expected["summary"]["partition_closure_edge_count"],
            "partition_closure_node_count": expected["summary"]["partition_closure_node_count"],
            "total_virtual_tensor_state_count": expected["summary"][
                "total_virtual_tensor_state_count"
            ],
        },
        "member_binding": expected["member_binding"],
        "method_parameter_registry": expected["method_binding"]["registry"],
        "partition_closure": {
            key: expected["partition_closure"][key]
            for key in ("domain", "edge_count", "node_count", "sha256")
        },
        "replay_binding": expected["replay_binding"],
        "runtime_binding": {
            "implementation_runtime_closure": protocol.entry["implementation_runtime_closure"],
            "observed_runtime": _runtime(),
            "verifier": {
                "path": str(images["verifier"].path),
                "sha256": images["verifier"].sha256,
            },
        },
        "schema": RECEIPT_SCHEMA,
        "status": RECEIPT_STATUS,
        "stream_binding": expected["axis_stream"],
    }
    publication_images = (
        observed,
        protocol.request_image,
        protocol.plan_image,
        protocol.bundle_image,
        protocol.commitment_image,
        *protocol.plan_dependency_images,
        *images.values(),
    )

    def revalidate_publication_images() -> None:
        for image in publication_images:
            _revalidate_file_image(image)

    _exclusive_publish_receipt(
        receipt_path,
        canonical_bytes(receipt),
        revalidate=revalidate_publication_images,
    )
    return receipt
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        request_path, artifact_path, receipt_path = _parse_cli(argv)
        receipt = validate(request_path, artifact_path, receipt_path)
