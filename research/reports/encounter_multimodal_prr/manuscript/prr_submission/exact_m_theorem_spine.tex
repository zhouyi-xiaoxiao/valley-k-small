% Reader-facing theory fragment for the main-article manuscript.
% It contains no finite-parameter numerical claim and is not input by the
% historical working manuscript.

\subsection{Prescribed finite modality from separated exposure clocks}
...
$\varepsilon$ and allocation set
$\mathcal A\subseteq\mathcal W_{w_*}$, define the connected
topology-retention budget
\begin{align}
...
\end{equation}
Consequently the budget-rescaled Doi reaction-time density has the stated
signature for every
$0<B<B_{\rm top}^{\rm unif}(\varepsilon)$ and every admissible allocation.
Multiplication by $B$ leaves the stationary points unchanged, so the
conclusion also holds for the physical density
