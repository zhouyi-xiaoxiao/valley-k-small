...
Here, \emph{channel mass balance} means that neither arrival channel is
negligible: the early channel must be high enough to form the first peak, but
not so dominant that it erases the delayed peak. \emph{Timescale separation}
means that the two channels reach the target on sufficiently different
typical time ranges, so their peaks do not merge into a single broad maximum.
...
\vspace{0.4em}
\includegraphics[width=0.82\linewidth]{shortcut_ring_k2_k4_beta001.pdf}
...
In the figures and scans below, a channel is a labelled source of first-passage
probability, such as shortcut versus non-shortcut paths, target 1 versus target
2, or near-target versus far-target arrivals. Channel mass balance asks how
much probability each channel contributes around the relevant peak windows.
Timescale separation asks whether the peak windows of those channels are far
enough apart to create a valley. The quantitative visual check is then the R3
ratio \(\rho=h_v/\min(h_1,h_2)\le0.8\).
