#!/usr/bin/env python3
"""Path density plots for v12 (density background + few representative paths)."""
...
def plot_paths_density_v12(
    ...
    *,
    ...
    if case.corridor and case.classification_rule:
        ...
        if rows:
            if roi is None:
                roi = roi_bounds_strip(case, corridor_rows=rows, pad_x=1, pad_y=1, min_width=10, min_height=4)
            use_strip = True
            full_view = strip_view_full_width(case, rows, pad_y=1)

    fig_width = 9.6 if use_strip else 12.0
    fig_height = 3.0 if use_strip else 4.6
    ...
    step_roi = 160 if is_slow else 80
