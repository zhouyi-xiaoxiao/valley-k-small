#!/usr/bin/env python3
"""Shared Fig.3-style drawing utilities for v8 plots."""
...
def save_clean(fig: plt.Figure, outpath: Path, *, dpi: int = 600) -> None:
...
def draw_door(
    ...
    *,
    ...
    for edge in door_segments:
        xs, ys = _edge_segment(edge)
        ax.plot(xs, ys, color="black", lw=2.6, ls=(0, (3, 2)), zorder=6)
...
def draw_corridor_band(
    ...
    *,
    ...
    y0 = int(corridor["y"]) - band_halfwidth
    height = 2 * band_halfwidth + 1
    rect = Rectangle(
        (x0 - 0.5, y0 - 0.5),
        (x1 - x0 + 1),
        height,
...
def detect_bimodality(y: np.ndarray, *, smooth_w: int = 7, min_sep: int = 5) -> dict:
    ys = smooth_ma(np.asarray(y, dtype=np.float64), smooth_w)
    ...
    return {
        ...
        "h2": h2,
        "hv": hv,
        "ratio": h2 / h1 if h1 > 0 else np.nan,
        "valley": hv / denom,
    }


def heatmap_norm(mats: Sequence[np.ndarray], *, eps: float = 1e-14) -> LogNorm:
    vals = np.concatenate([m.ravel() for m in mats])
    vals = vals[np.isfinite(vals)]
    vals = vals[vals > eps]
    vmin = eps if vals.size == 0 else max(eps, float(np.quantile(vals, 1e-4)))
...
def add_time_tag(ax: plt.Axes, t: int) -> None:
    ax.text(
        0.95,
        0.06,
        f"t={t}",
        ...
        fontsize=10,
        color="white",
        bbox=dict(boxstyle="round,pad=0.25", facecolor="black", alpha=0.4, linewidth=0.0),
    )
...
def write_gallery_html(fig_root: Path) -> None:
    ...
    for rel, name in items:
        html_lines.append(f"<div class='item'><div><code>{rel}</code></div><img src='{rel}' alt='{name}'></div>")
