#!/usr/bin/env python3
"""Detached H12 controller: H11 recovery plus isolated scheduler and job environments."""
...
MAN = "notes/isambard_ai_v4_r2_h12_payload.sha256"
H11_MAN = "notes/isambard_ai_v4_r2_h11_payload.sha256"
RUNTIME = "code/h12_runtime_v4_r2.py"
ZERO = "0" * 64
...
def load_h11_controller():
    ...
    module = types.ModuleType("h12_pinned_h11_controller_base")
...
CAMPAIGN_ORDER = ("v3_authority", "canary", "production")
SCIENCE_SCRIPTS = dict(base.SCRIPTS)
SCRIPTS = {phase: SCIENCE_SCRIPTS[phase] for phase in CAMPAIGN_ORDER}
SELFTEST_PHASES = ("selftest_upstream", "selftest_downstream")


def verify_package(root: Path) -> dict[str, str]:
    req(HEX.fullmatch(H12_SHA) is not None and H12_SHA != ZERO, "H12 pin unset")
    root = trusted_abs(root)
    req(root.is_dir() and stat.S_IMODE(root.lstat().st_mode) == 0o700, "package root mode")
    manifest = root / MAN
    req(manifest.is_file() and not manifest.is_symlink() and sha(manifest) == H12_SHA, "externally pinned H12 drift")
    ...
    req(actual == set(rows) | {MAN}, "closed H12 package inventory")
...
def python_anchor(phase: str) -> dict:
    del phase
    return secure_root_executable(Path(sys.executable))
...
def render(
    ...
    lines = [
        ...
        *directives,
        ...
        'launch="$(/usr/bin/mktemp -d "$SLURM_TMPDIR/h12-launch.XXXXXX")"',
...
def hardened_base(anchors: dict[str, dict]):
    names = ("verify_package", "render", "prepare_intent", "subprocess")
...
def envelope_path(run: Path, phase: str, job: str) -> Path:
    return run / "artifacts/h12_envelopes" / f"{phase}-{job}.json"
...
def validate_envelope(run: Path, phase: str, job: str) -> dict:
    ...
    req(
        value.get("schema") == "h12-execution-envelope-v1"
        and value.get("status") == "PASS_H12_ISOLATED_ENVIRONMENT_AND_PINNED_EXECUTABLES"
        and value.get("phase") == phase
        and value.get("job_id") == job
        and value.get("h12") == H12_SHA
        and value.get("h11") == H11_SHA,
        "H12 envelope identity",
    )
    req(
        ...
        "H12 envelope legacy receipt binding",
    )
    ...
    req(
        ...
        "H12 envelope submission policy",
    )
    return value


def validate_h12_prior(run: Path, phase: str, job: str) -> None:
    ...
    for path in paths:
        value = base.json_mode(path)
        req(
            value.get("h12") == H12_SHA
            and value.get("h12_status") == "PASS_H12_ENVIRONMENT_ISOLATED_EXECUTION",
            "prior runtime lacks H12 environment authority",
        )
...
def submit(
    ...
    with controller_lock(run):
        ...
        if phase == "selftest_downstream":
            ...
            validate_h12_prior(run, "selftest_upstream", dependency)
        ...
        value = {
            "schema": "h12-execution-envelope-v1",
            "status": "PASS_H12_ISOLATED_ENVIRONMENT_AND_PINNED_EXECUTABLES",
            "phase": phase,
            "job_id": job,
        ...
        path = envelope_path(run, phase, job)
        if path.exists():
            req(base.json_mode(path) == value, "existing H12 envelope drift")
        else:
            base.exclusive(path, value)
        validate_envelope(run, phase, job)
        return {"h12": value, **result}


def finalize(run: Path, jobs: dict) -> dict:
    run = trusted_abs(run)
    req(set(jobs) == set(CAMPAIGN_ORDER), "final H12 campaign job set")
    selected = tool_anchors()
    with controller_lock(run):
        previous = None
        phases = []
        with hardened_base(selected):
            for phase in CAMPAIGN_ORDER:
                job = str(jobs[phase])
                req(job.isdecimal(), "final H12 job identity")
                validate_h12_prior(run, phase, job)
                gate = base.prior_gate(run, phase, job, selected["sacct"]["path"])
                req(
                    gate["submission"]["dependency_afterok"] == previous,
                    "final H12 dependency chain",
                )
                phases.append(
                    {
                        "phase": phase,
                        "job_id": job,
                        "h12_envelope_sha256": sha(envelope_path(run, phase, job)),
                ...
                previous = job
        value = {
            "schema": "grid2d-v4-r2-h12-production-campaign-candidate-v1",
            "status": "PASS_H12_960_NHR_CEILING_CAMPAIGN_NO_SCIENTIFIC_AUTHORITY",
            "h12": H12_SHA,
            "h11": H11_SHA,
            "phases": phases,
            "production_shape": {
        ...
        output = run / "artifacts/h12_final/production-campaign-candidate.json"
