#!/usr/bin/env python3
"""Verify H2 four-GPU canary against the H2 v3 release authority."""
...
def main() -> int:
    ...
    science.req(release.get("schema") ==
                ...
                and release.get("authorizes_v4_r2_h2") is True,
                "H2 release did not authorize GPU canary")
    expected = {f"lane-{lane}.json" for lane in range(4)}
    science.req({member.name for member in args.lanes.iterdir()} == expected,
                "H2 lane exact inventory drift")
    rows = []
    for lane in range(4):
        ...
        science.req(value.get("schema") ==
                    ...
                    "H2 lane schema/index drift")
        rows.append(value)
    uuids = {row["gpu"]["uuid"] for row in rows}
    pci = {row["gpu"]["pci_bus_id"] for row in rows}
    visible = {row["cuda_visible_devices"] for row in rows}
    science.req(len(uuids) == len(pci) == len(visible) == 4,
                "H2 canary did not prove four distinct GPU identities")
    payload = {
        ...
        "lanes": [{
            ...
            "uuid": row["gpu"]["uuid"], "pci_bus_id": row["gpu"]["pci_bus_id"],
    ...
    science.req(not args.output.exists(), "H2 canary receipt exists")
    raw = (json.dumps(payload, sort_keys=True, indent=2, allow_nan=False) + "\n").encode()
    descriptor, name = tempfile.mkstemp(prefix=".canary-h2.", dir=args.output.parent)
    temporary = Path(name)
    try:
