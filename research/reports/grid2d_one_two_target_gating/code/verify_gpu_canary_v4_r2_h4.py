#!/usr/bin/env python3
"""Verify the H4 four-GPU canary against the exact H4 v3 authority."""
...
def main() -> int:
    ...
    science.req(args.release_receipt == v3_h4.H4_RELEASE
                and science.sha(args.release_receipt) == args.release_sha256,
                "H4 release receipt path/hash drift")
    release = science.strict_json(args.release_receipt, mode600=True)
    science.req(set(release) == v3_h4.TOP_KEYS
                and release["schema"] == v3_h4.SCHEMA
                and release["status"] == v3_h4.STATUS
                and release["authorizes_v4_r2_h4"] is True,
                "H4 release did not authorize GPU canary")
    expected = {f"lane-{lane}.json" for lane in range(4)}
    science.req({item.name for item in args.lanes.iterdir()} == expected,
                "H4 lane exact inventory drift")
    rows = []
    for lane in range(4):
    ...
    science.req(len({row["gpu"]["uuid"] for row in rows}) == 4
                ...
                "H4 canary did not prove four distinct GPUs")
    payload = {
    ...
    science.req(not args.output.exists(), "H4 canary receipt exists")
    raw = (json.dumps(payload, sort_keys=True, indent=2, allow_nan=False) + "\n").encode()
    descriptor, name = tempfile.mkstemp(prefix=".canary-h4.", dir=args.output.parent)
    temporary = Path(name)
    try:
