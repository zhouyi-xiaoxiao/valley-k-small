...
Second, no parameter choice can make a \(t\lambda^t\) term survive in this model at all.  For \(t\geq1\) the first-passage probability is a linear functional of powers of the transient submatrix of the shortcut chain (all rows and columns except the absorbing site \(v\)).  That submatrix is \emph{symmetric}: starting from the symmetric killed ring matrix, the directed shortcut only lowers the diagonal entry at \(u\), because the reallocated mass \(\beta(1-q)\) leaves through the absorbing column.  A symmetric matrix is orthogonally diagonalizable with real spectrum, and a diagonalizable finite chain produces a pure sum of geometric modes \(\sum_i\kappa_i\lambda_i^t\); a secular term \(t\lambda^t\) would require a defective (Jordan-block) eigenvalue, which a symmetric matrix cannot have.  The spectrum of the transient submatrix is exactly \(\{\gamma_r\}_{r=1}^{L-1}\cup\{s_j\}_{j=1}^{L}\): the even (\(\gamma\)) eigenvectors vanish at \(u\) and are untouched by the rank-one defect, while the odd modes move from \(\alpha_\ell\) to the roots \(s_j\) of \(D(y)\), strictly interlacing the \(\alpha_\ell\) for every \(\beta>0\).  Hence for \(\beta>0\) the values \(\alpha_\ell\) are not in the spectrum, and a \(t\alpha_\ell^t\) contribution is impossible rather than merely cancelled.  (In the antipodal geometry the \(\gamma_r\)-modes also carry zero absorption flux, which is why Eq. (\ref{eq:tail_residue}) contains only the \(s_j\); see the note after Eq. (\ref{eq:app_W_modes}).)
