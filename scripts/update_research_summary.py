...
ALLOWED_EXTRA_PDF_PATTERNS = ("note_*.pdf", "method_comparison*.pdf", "fig*_description*.pdf")
ALLOWED_EXTRA_TEX_PATTERNS = ("note_*.tex", "method_comparison*.tex")
...
def _collect_registry_rows(root: Path) -> list[dict[str, Any]]:
    ...
    for item in load_registry():
        ...
        canonical_pdfs = [n for n in _canonical_pdf_names(item) if (report_dir / n).exists()]
        extra_pdfs = [n for n in _list_extras(report_dir, patterns=ALLOWED_EXTRA_PDF_PATTERNS) if n not in canonical_pdfs]
        pdfs = canonical_pdfs + extra_pdfs

        canonical_tex = [str(t) for t in item.get("main_tex", []) if (report_dir / str(t)).exists()]
        extra_tex = [n for n in _list_extras(report_dir, patterns=ALLOWED_EXTRA_TEX_PATTERNS) if n not in canonical_tex]
...
def render_table_rows(items: Iterable[dict[str, Any]]) -> list[str]:
    ...
    for row in items:
        ...
        texs = list(row["tex"])
        pdf_cell = (
            ", ".join(f"`{rel}/{name}`" for name in pdfs) if pdfs else "-"
        )
        tex_cell = (
            ", ".join(f"`{rel}/{name}`" for name in texs) if texs else "-"
        )
...
def append_archive_log(root: Path, archived_items: list[str], today: str) -> None:
    if not archived_items:
        return
    log_path = root / "docs" / "research_log" / "RESEARCH_PROGRESS_ARCHIVE.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Refresh docs/RESEARCH_SUMMARY.md date and auto index."
    )
    parser.add_argument(
        "--summary",
        default="docs/RESEARCH_SUMMARY.md",
