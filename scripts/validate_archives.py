...
def validate_archives(root: Path, *, strict: bool = False) -> list[str]:
    errs: list[str] = []
    index_schema = json.loads((root / "schemas" / "archive_index.schema.json").read_text(encoding="utf-8"))
    latest_schema = json.loads((root / "schemas" / "latest_manifest.schema.json").read_text(encoding="utf-8"))
    valid_report_ids = {item["id"] for item in load_registry()}

    index_path = root / "archives" / "reports" / "index.jsonl"
    ...
    manifests = sorted(root.glob("reports/*/**/latest/manifest.json"))
    if strict and not manifests:
        errs.append("no latest manifests found under reports/*/**/latest/manifest.json")
    for mf in manifests:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Validate archives/reports index + latest manifests")
