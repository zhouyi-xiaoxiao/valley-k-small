...
def validate_payload(payload: dict[str, Any], *, root: Path) -> list[str]:
    ...
    for idx, item in enumerate(reports):
        ...
        if not PATH_RE.match(rpath):
            ...
            continue
        if not rpath.startswith("reports/"):
            errors.append(f"{where}.path must start with reports/: {rpath!r}")
        ...
        for tex in item.get("main_tex", []):
            ...
            if not TEX_RE.match(tex):
                ...
                continue
            if report_dir.exists() and not (report_dir / tex).is_file():
                errors.append(f"{where}.main_tex file missing: {rpath}/{tex}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Validate reports/report_registry.yaml")
...
def main() -> int:
    args = parse_args()
    root = repo_root()
    schema_path = root / "schemas" / "report_registry.schema.json"
