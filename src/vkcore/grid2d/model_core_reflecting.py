...
class ConfigSpec:
    ...
    local_bias_deltas: Dict[Coord, float] = field(default_factory=dict)
...
def scale_spec(spec: ConfigSpec, N_new: int) -> ConfigSpec:
    ...
    local_bias_deltas = {scale_coord(k, scale, N_new): v for k, v in spec.local_bias_deltas.items()}
...
def spec_to_internal(spec: ConfigSpec) -> LatticeConfig:
    ...
    local_bias_deltas = {coord_to0(k): float(v) for k, v in spec.local_bias_deltas.items()}
...
def config_to_dict(spec: ConfigSpec) -> dict:
    return {
        ...
        "local_bias_delta": spec.local_bias_delta,
        "n_local_bias_deltas": int(len(spec.local_bias_deltas)),
...
def move_probs(cfg: LatticeConfig, xy: Coord, include_local: bool = True) -> Tuple[float, float, float, float, float]:
    ...
    if include_local:
        ...
        delta = cfg.local_bias_deltas.get(xy, cfg.local_bias_delta)
        if arrow is not None and delta > 0.0 and p_stay > 0.0:
            shift = delta * p_stay
            p_stay -= shift
            if arrow == "left":
                p_left += shift
